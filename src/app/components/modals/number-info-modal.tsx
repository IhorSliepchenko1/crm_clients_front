...
export const NumberInfoModal: React.FC<Props> = ({
  opened,
  close,
  numberInfo,
  dataCity,
  ...
  const form = useForm({
    ...
    validate: {
      dob: (value: string | any[]) =>
        value && (value.length > 4 || value.length < 4)
          ? "Вы должны ввести только год рождения!"
          : null,
    },
  });
  ...
  const hiatoriesCall = useMemo(() => {
    ...
    histories.forEach((item) => {
      const dateEvent = `Дата время встречи: ${
        item.presentation_date && item.presentation_time
          ? formatDate(item.presentation_date, false) +
            " " +
      ...
      const itemArray = [
        `Дата звонка: ${
          item.call_date ? formatDate(item.call_date, false) : "-"
        }`,
  ...
  return (
    <Modal
      opened={opened}
      onClose={close}
      title={`Информация о номере: ${number}`}
      size="100%"
    >
      <form
